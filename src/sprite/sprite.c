...
#define conditions                              \
    ...
    obj1.origin = (vect2){sp1->x, sp1->y};
    obj1.extremity = (vect2){sp1->x + sp1->width, sp1->y + sp1->height};

    obj2.origin = (vect2){sp2->x, sp2->y};
    obj2.extremity = (vect2){sp2->x + sp2->width, sp2->y + sp2->height};
...
sprite **all_collided(sprite *sp, int lid)
{
    ...
    if(!sp || lid < 0 || lid > MAX_LAYERS) return NULL;
